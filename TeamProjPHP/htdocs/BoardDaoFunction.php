...
function insert($dbname)
{
    ...
    $sql = "INSERT INTO "+$dbname+" (board_pw, board_title,
    ...
    try {
        ...
        if ($row = mysqli_fetch_array($result)) {
            ...
            $deleteSql = "DELETE FROM board WHERE board_pw='"
                ...
                echo "<script>alert(\"삭제 성공!\");location.replace('http://localhost:8655/PHP_BOARD2_Test/htdocs/board_list.php');</script>";
        }
        // 패스워드가 맞는 해당 자료가 없으면
        else {
            echo "<script>alert(\"삭제 실패! 비밀번호를 다시 확인해 주세요\");location.replace('http://localhost:8655/PHP_BOARD2_Test/htdocs/board_list.php');</script>";
        }
