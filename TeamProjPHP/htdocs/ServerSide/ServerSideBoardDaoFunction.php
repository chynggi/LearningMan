...
function delete(){
    ...
    try {
        // 삭제대상자료가 있는 지 확인
        $pdoStatement = $conn -> prepare("SELECT * FROM ssboard WHERE no= :no");
        $pdoStatement -> bindValue(":no", $board_no);
        $pdoStatement -> execute();
        // 패스워드가 맞는 해당 자료가 있으면
        if ($row = $pdoStatement -> fetch()) {
            // 지우는 작업            
            $pdoStatement = $conn -> prepare("DELETE FROM ssboard WHERE no= :no");
            $pdoStatement -> bindValue(":no", $board_no);
            $pdoStatement -> execute();                
        }
        // 패스워드가 맞는 해당 자료가 없으면
...
function update(){
    ...
    $conn = dbConnect();         
    $pdoStatement = $conn -> prepare("UPDATE ssboard SET title=? ,content=? WHERE no=?)");
    $pdoStatement -> bindValue(1, $board_title);
    $pdoStatement -> bindValue(2, $board_content);
...
function selectAll(){
    $conn = dbConnect();
    $pdoStatement = $conn -> prepare("SELECT no, title, user, xdate FROM ssboard order by no desc");
    $results = $pdoStatement -> fetchAll();
    return $results;
}
function selectOne($key){
    $conn = dbConnect();
    $pdoStatement = $conn -> prepare("SELECT no, title, user, xdate, content FROM ssboard where no=?");
